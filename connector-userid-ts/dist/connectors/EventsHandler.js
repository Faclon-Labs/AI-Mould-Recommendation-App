...
import { PUBLISH_EVENT_URL, GET_EVENTS_IN_TIMESLOT_URL, GET_EVENT_DATA_COUNT_URL, GET_EVENT_CATEGORIES_URL, GET_DETAILED_EVENT_URL, GET_MONGO_DATA, GET_MAINTENANCE_MODULE_DATA, GET_DEVICE_DATA, GET_SENSOR_ROWS, GET_DEVICE_METADATA_MONGO_URL, Protocol, VERSION } from '../utils/constants.js';
import DataAccess from './DataAccess.js';
export default class EventsHandler {
    constructor({ userId, dataUrl, onPrem = false, tz = 'UTC', logTime = false }) {
    ...
    async publishEvent(options) {
         ...
         */
        try {
            const { message, metaData, hoverData, createdOn, eventTagsList, eventNamesList, title, onPrem } = options;
            ...
            if (eventNamesList && eventNamesList.length > 0) {
                ...
                for (const tagName of eventNamesList) {
                    const matched = categories.find((item) => item.name === tagName);
                    if (!matched) {
    ...
    async getEventsInTimeslot(options) {
        /**
        ...
        try {
            ...
            if (new Date(endTimeIso) < new Date(startTimeIso)) {
                throw new Error(`Invalid time range: start_time(${startTimeIso}) should be before end_time(${endTimeIso}).`);
            }
    ...
    async getEventDataCount(options = {}) {
        /**
    ...
    async getDetailedEvent(options = {}) {
        /**
        ...
        try {
            ...
            if (!finalEventTagsList) {
                const categories = await this.getEventCategories({ onPrem });
                finalEventTagsList = categories.map((category) => category._id);
            }
    ...
    async getPaginatedData(url, payload, parallel) {
        /**
    ...
    async getMaintenanceModuleData(options) {
        /**
         * Fetch maintenance module data based on the provided parameters.
         */
        try {
            const { startTime, endTime, remarkGroup, eventId, maintenanceModuleId, operator, dataPrecision, periodicity, cycleTime, weekStart, monthStart, yearStart, shifts, shiftOperator, filter, onPrem } = options;
    ...
    async getDeviceMetadata(deviceId, onPrem) {
        /**
